...
<div class="flex items-center justify-between mb-6">
    <div class="flex items-center">
        ...
        <h2 class="text-xl md:text-2xl font-bold text-tech-dark">Modifier: {{ Str::limit($post->title, 30) }}</h2>
    </div>
</div>

<div class="bg-white rounded-xl shadow-lg overflow-hidden">
    <form action="{{ route('admin.blog.update', $post) }}" method="POST" enctype="multipart/form-data" class="p-4 md:p-6 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text"
                   ...
                   class="w-full px-4 md:px-6 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-tech-blue focus:border-transparent transition-colors @error('title') border-red-600 @enderror"
        ...
        <div>
            ...
            <textarea name="excerpt"
                      id="excerpt"
                      rows="3"
                      class="w-full px-4 md:px-6 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-tech-blue focus:border-transparent transition-colors resize-none @error('excerpt') border-red-600 @enderror"
        ...
        <div>
            ...
            <textarea name="content"
                      id="content"
                      rows="12"
                      class="w-full px-4 md:px-6 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-tech-blue focus:border-transparent transition-colors resize-y @error('content') border-red-600 @enderror"
        ...
        <div>
            ...
            <div id="imagePreview" class="mt-4 hidden">
                <div class="flex items-center">
                    <img id="previewImg" src="" alt="Aperçu" class="w-16 h-16 object-cover rounded-lg mr-3">
                    <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <select name="status"
                        id="status"
                        class="w-full px-4 md:px-6 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-tech-blue focus:border-transparent transition-colors @error('status') border-red-600 @enderror"
                        required>
                ...
                <!-- Statut Preview -->
                <div class="mt-2">
                    <span id="statusPreview" class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $post->status === 'published' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
            ...
            <div>
                ...
                <input type="date"
                       ...
                       class="w-full px-4 md:px-6 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-tech-blue focus:border-transparent transition-colors @error('published_at') border-red-600 @enderror">
                @error('published_at')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
        ...
        <!-- Boutons d'action -->
        <div class="flex flex-col md:flex-row items-center justify-end space-y-4 md:space-y-0 md:space-x-4 pt-6 border-t border-gray-200">
            <a href="{{ route('admin.blog.index') }}"
               class="w-full md:w-auto px-6 py-3 border border-gray-200 rounded-lg text-gray-500 font-semibold hover:bg-gray-50 transition-colors text-center">
                Annuler
            </a>
            <button type="submit"
                    class="w-full md:w-auto bg-tech-blue text-white px-6 py-3 rounded-lg font-semibold hover:bg-tech-blue/80 transition-colors">
